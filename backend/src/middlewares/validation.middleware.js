...
const customValidations = {
    // Validate route id parameters used by MongoDB and Prisma records
    validateObjectId: (paramName = 'id') => {
        return (req, res, next) => {
            ...
            if (!id || ![objectIdPattern, cuidPattern, cuid2Pattern, uuidPattern, legacyImportPattern].some(pattern => pattern.test(id))) {
                return res.status(400).json({
                    success: false,
                    message: `Invalid ${paramName} format`,
                    errors: [{
                        field: paramName,
                        message: `${paramName} must be a valid record id`,
                        value: id
                    }]
                });
            }
