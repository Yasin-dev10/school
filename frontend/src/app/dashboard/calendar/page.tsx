...
export default function CalendarPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [eventRes, classRes] = await Promise.all([api.get('/calendar/events', { params: { from: from.toISOString(), to: to.toISOString() } }), api.get('/classes').catch(() => ({ data: { data: [] } }))]);
  ...
  return <div className="max-w-7xl mx-auto space-y-5">
    <header className="flex flex-col sm:flex-row sm:items-center justify-between gap-3"><div><h1 className="text-2xl font-bold flex items-center gap-2"><CalendarDays className="text-indigo-500" /> School Calendar</h1><p className="text-sm text-slate-500">Holidays, exams, meetings and school events.</p></div>{canCreate && <button onClick={() => setShowCreate(true)} className="px-4 py-2.5 rounded-xl bg-indigo-600 text-white font-semibold flex items-center gap-2"><Plus className="w-4 h-4" /> Add event</button>}</header>
